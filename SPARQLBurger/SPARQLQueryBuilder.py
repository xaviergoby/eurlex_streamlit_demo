...
class SPARQLGraphPattern:
    def __init__(self, optional=False, union=False, name=None):
        """
    ...
    def add_named_graph_pattern(self, graph_pattern):
        pass
    ...
    def get_text(self, indentation_depth=0):
        ...
        try:
            ...
            #     query_text = f"{outer_indentation}graph ?{self.name}" + " { \n"
            ...
            if self.is_optional:
                ...
                # query_text = f"{outer_indentation}" + "{\n" + f"{outer_indentation}graph ?{self.name}" + " { \n"
                query_text = f"{outer_indentation}graph ?{self.name}" + " { \n"
            else:
                query_text = "%s{\n" % (outer_indentation, )
...
class SPARQLQuery:
    ...
    def add_prefix(self, prefix):
        ...
        if type(prefix) is Prefix:
            ...
            prefix_key, namespace_val = list(prefix.items())[0]
            self.prefixes.append(Prefix(prefix=prefix_key, namespace=namespace_val))
            return True
            # prefix_dict_keys = [k.lower() for k in list(prefix.keys())]
            # if "prefix" in prefix_dict_keys and "namespace" in prefix_dict_keys:
            #     self.prefixes.append(Prefix(prefix=prefix["prefix"], namespace=prefix["namespace"]))
            #     return True
            # else:
            #     self.prefixes.append(Prefix(prefix=prefix_dict_keys, namespace=prefix["namespace"]))
            # return True
        else:
            return False
        
    def add_prefixes(self, prefixes):
        for prefix_key, namespace_val in prefixes.items():
            # self.prefixes.append(prefix)
            self.prefixes.append(Prefix(prefix=prefix_key, namespace=namespace_val))
...
class SPARQLSelectQuery(SPARQLQuery):
    def __init__(self, distinct=False, limit=False, order_by="desc", include_popular_prefixes=False):
        """
    ...
    def get_text(self, indentation_depth=0):
        ...
        try:
            ...
            # if self.limit and self.order_by:
            #     query_text += f"\nORDER BY {self.order_by}(?date)"
